...
@static if is_unix() const libCLBLAS = "libclBLAS" end
@static if is_windows() const libCLBLAS = "clBLAS" end
...
function setup(profile=false)
    ...
    available_devices = @static if is_apple() cl.devices(:gpu) else cl.devices() end
